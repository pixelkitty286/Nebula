/datum/map/prometheus/send_welcome()
	var/obj/effect/overmap/visitable/ship/prometheus = SSshuttle.ship_by_type(/obj/effect/overmap/visitable/ship/prometheus)

	var/welcome_text = "<center><font size = 3><b>[global.using_map.station_name]</b> Sensor Readings:</font><br>"
	welcome_text += "Report generated on [stationdate2text()] at [stationtime2text()]</center><br /><br />"
	welcome_text += "<hr>Current system:<br /><b>[prometheus ? system_name() : "Unknown"]</b><br /><br>"

	if(prometheus) //If the overmap is disabled, it's possible for there to be no prometheus.
		var/list/space_things = list()
		welcome_text += "Current Coordinates:<br /><b>[prometheus.x]:[prometheus.y]</b><br /><br>"
		...
		for(var/obj/effect/overmap/visitable/O in space_things)
			...
			if(O.loc != prometheus.loc)
				var/bearing = round(90 - Atan2(O.x - prometheus.x, O.y - prometheus.y),5) //fucking triangles how do they work
				if(bearing < 0)
					bearing += 360
